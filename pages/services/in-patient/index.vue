<template>
	...
	<v-card class="mb-2" elevation="4">
		...
		>
			<template v-for="column in columns" v-slot:[`column-${column.key}`]="{ item }">
				...
				<span 
					:style="{ cursor: 'default', display: 'block', height: '26px', width: '9px', backgroundColor: item.patient_registry && item.patient_registry[0].guarantor_Name !== 'Self Pay' ? 'yellow' : 'orange' }" 
					:title="item.patient_registry && item.patient_registry[0].guarantor_Name !== 'Self Pay' ? 'HMO ' : 'Self Pay'"
					/>
				</div>
	...
	<CentralLookUpForm 
		...
		@search="SearchInpatient"
		@selected-row="selectedInpatient"
	...
	<DebitRoomChargesDialog :show="DebitRoomCharges" @close-dialog="useSubComponents('DebitRoomCharges', false)" />
