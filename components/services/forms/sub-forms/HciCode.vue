<template>
    <v-dialog :model-value="open_referring_hci_code_table" rounded="lg" @update:model-value="closeDialog" scrollable max-width="700px">
        <form @submit.prevent="onSubmit">
            <v-card rounded="lg">
                <v-toolbar density="compact" color="#6984ff" hide-details>
                    <v-toolbar-title>Select HCI Referring Center</v-toolbar-title>
                ...
                <v-card-actions>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn class="bg-primary text-white" type="submit">Select Institution</v-btn>
                </v-card-actions>
            </v-card>
        </form>
    </v-dialog>
...
const headers = [
    ...
    { title: 'Referred ICHP', key: 'referred_ichp', align: 'start', width:"30%" },
];
const data = ref({
...
const onSubmit = () => {
    if (selected_item.value.length === 0) {
        alert("Please select a HCI Referring Center");
        return;
    }
