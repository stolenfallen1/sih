<template>
    <v-dialog :model-value="open_returned_supply" rounded="lg" @update:model-value="closeDialog" max-width="1120px">
        <v-card rounded="lg">
            <v-toolbar density="compact" color="#107bac" hide-details>
                <v-toolbar-title>
                    Post Returned Supplies
    ...
    <v-dialog :model-value="open_selected_items_dialog" rounded="lg" max-width="950px" @update:model-value="closeSelectedItemsDialog">
        <v-card elevation="4" rounded="lg">
            ...
            <v-card-actions>
                ...
                <v-spacer></v-spacer>
                <v-btn class="bg-primary text-white" @click="openReturnSuppliesConfirmation">Return Supplies</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
...
const headers = [
    { title: "Trans Date",  align: "start", sortable: false, key: "transaction_Date" },
    { title: "Encoder",  align: "start", sortable: false, key: "transaction_UserID" },
    { title: "Item ID",  align: "start", sortable: false, key: "transaction_Item_Id" },
...
const getPatientPostedSupplies = async () => {
    ...
    try {
        const response = await useMethod("get", "get-ancillary-posted-supplies?case_No=", "", payload.value.case_No);
        if (response) {
...
const openReturnSuppliesConfirmation = () => {
    ...
    if (payload.value.remarks == null || payload.value.remarks == "") {
        useSnackbar(true, "error", "Please provide a reason for returning the supply.");
        return;
    }
...
const onSubmitReturnSupplies = async (user_details) => {
    if (user_details.user_passcode === usePasscode()) {
        payload.value.patient_Type = props.patient_type;
        const response = await useMethod("post", "post-return-supplies", payload.value);   
        if (response) {
            useSnackbar(true, "success", "Supplies returned successfully.");
