<template>
    <v-dialog :model-value="open_ancillary_manual_posting" rounded="lg" @update:model-value="closeDialog" max-width="1120px">
        <v-card rounded="lg">
                            ...
                            <div v-if="payload.patient_Name != null" >
                                <v-btn class="mr-4" density="compact">
                                    ...
                                    <v-menu activator="parent">
                                        <v-list>
                                            <v-list-item
                                                v-for="(item, index) in supply_revenue_data"
                                                :key="index"
                                                :value="index"
                                            >
                        ...
                        <v-table density="compact" height="40vh" class="styled-table">
                                ...
                                <tbody>
                                    <template v-for="(item, index) in Supplies">
                                        <tr>
                                            <td> <input class="input supply-focus" v-model="item.code" @keyup.enter="handleAddSupply(item, index)" :readonly="item.isReadonly" /> </td>
                                            <td> <input class="input supply-focus" v-model="item.map_item_id" @keyup.enter="handleAddSupply(item, index)" :readonly="item.isReadonly" /> </td>
                                            <td> <input class="input supply-focus" v-model="item.item_name" @keyup.enter="handleAddSupply(item, index)" readonly /> </td>
                                            <td> <input class="input supply-focus" v-model="item.item_OnHand" @keyup.enter="handleAddSupply(item, index)" readonly /> </td>
                                            <td> 
                                                <input 
                                                    class="input supply-focus" 
                                                    v-model="item.quantity" 
                                                    @input="updateAmount(item)"
                                                /> 
                                            </td>
                                            <td> <input class="input supply-focus" v-model="item.price" readonly /> </td>
                                            <td> <input class="input supply-focus" v-model="item.amount" readonly /> </td>
                                            <td>
                                                <select class="input supply-focus" v-model="item.stat">
                                                    <option value="1">Routine</option>
                                                    <option value="2">Stat</option>
                                                </select>
                                            </td>
                                            <td v-if="!item.isAdd" class="cursor-pointer" ><v-icon @click="handleAddSupply(item, index)" color="primary">mdi-plus-box</v-icon></td>
                                            <td v-if="item.isDelete" class="cursor-pointer"><v-icon @click="removeSupplyItem(item)" color="red">mdi-delete</v-icon></td> 
                                        </tr>
                                    </template>
                                </tbody>
                                <tfoot>
...
const error_msg = ref("");
const supply_revenue_data = ref([]);
const supply_revenue_data_display = ref([]);
...
const getRevenueCode = async () => {
    ...
    if (revenue_res) {
        ...
        if (desiredCodes && Array.isArray(desiredCodes)) {
            supply_revenue_data.value = revenue_res.filter(item => {
                return item.id 
                    && item.isSupplies == "1"
                    && desiredCodes.map(code => code.toString()).includes(item.id.toString());
            });

            if (supply_revenue_data.value.length > 0 ) {
                selectedWarehouseID.value = supply_revenue_data.value[0].warehouse_id;
            }

            supply_revenue_data_display.value = supply_revenue_data.value.map(item => {
...
const focusNextSupply = (index) => {
    const inputs = document.querySelectorAll('.supply-focus');
...
const handleAddSupply = (item, index) => {
    item.code = item.code.toUpperCase();
    const lastRow = Supplies.value[Supplies.value.length - 1];
    if (!item.code) {
        return useSnackbar(true, "error", "Code Should not be empty.");
    }
    const desiredCodes = supply_revenue_data.value.map(item => item.code);
    if (desiredCodes.includes(item.code) === false) {
        return useSnackbar(true, "error", "Invalid Code, refer to help code.");
    }

    const matchingRevenue = supply_revenue_data.value.find(revenue => revenue.code === item.code);
    ...
    if(item.code && !item.map_item_id && !item.item_name) {
        ...
        let medicines = Supplies.value.filter(function (obj) {
        ...
        try {
            ...
            if (credit_limit.value != null && totalAmount.value > credit_limit.value) {
                ...
                if (!confirmCharge) {
                    ...
                    const response = await useMethod("post", "submit-manual-posting-supplies", payload.value);
                    if (response) {
...
const removeSupplyItem = (item) => {
    const index = Supplies.value.indexOf(item);
    if (index > -1) {
