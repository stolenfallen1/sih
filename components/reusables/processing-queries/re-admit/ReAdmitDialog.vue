    ...
    <v-dialog :model-value="show" rounded="lg" scrollable @update:model-value="closeDialog" max-width="1120px">
        <v-card rounded="lg">
            <v-toolbar density="compact" color="#6984ff" hide-details>
                <v-toolbar-title>
                    Inpatient Discharge List {{ selectedRowDetails.id }}
                </v-toolbar-title> 
            ...
            <v-card-actions>
                ...
                <v-spacer></v-spacer>
                <v-btn class="bg-primary text-white" @click="reAdmit">Continue</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
    <RegistryReadmissionProcess :open_readmit_dialog="open_readmit_dialog" @close-dialog="closeReAdmit" />
</template>

<script setup>
import RegistryReadmissionProcess from './sub-forms/RegistryReadmissionProcess.vue';
...
const payload = ref({});
const open_readmit_dialog = ref(false);
...
const reAdmit = () => {
    open_readmit_dialog.value = true;
}
const closeReAdmit = () => {
    open_readmit_dialog.value = false;
}
