<template>
    <v-dialog :model-value="show" rounded="lg" persistent scrollable max-width="700px">
        <v-toolbar density="compact" color="#6984ff" hide-details>
            <v-toolbar-title>Billing Report Category</v-toolbar-title>
            <v-spacer></v-spacer>
        ...
        <v-card>
            ...
            <v-card-actions>
                ...
                <v-spacer></v-spacer>
                <v-btn class="bg-primary text-white" type="submit" @click="openBillingReportForm">Add</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
    <billing-report-form :open_billing_form="open_billing_form" @close-dialog="closeBillingReportForm" @handle-submit="onSubmit" />
</template>

<script setup>
import BillingReportForm from './sub-forms/BillingReportForm.vue';
...
const openBillingReportForm = () => {
    open_billing_form.value = true;
}

const closeBillingReportForm = () => {
    open_billing_form.value = false;
}
