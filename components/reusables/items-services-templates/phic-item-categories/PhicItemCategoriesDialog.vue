<template>
  <v-dialog :model-value="show" rounded="lg" persistent scrollable max-width="700px">
      <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>PHIC Item Categories</v-toolbar-title>
          <v-spacer></v-spacer>
      ...
      <v-card>
          ...
          <v-card-actions>
              ...
              <v-spacer></v-spacer>
              <v-btn class="bg-primary text-white" type="submit" @click="openPhicItemCategoryForm">Add</v-btn>
          </v-card-actions>
      </v-card>
  </v-dialog>
  <phic-item-categories-form :open_phic_item_category_form="open_phic_item_category_form" @close-dialog="closePhicItemCategoryForm" @handle-submit="onSubmit" />
</template>

<script setup>
import PhicItemCategoriesForm from "./sub-forms/PhicItemCategoriesForm.vue";
...
const openPhicItemCategoryForm = () => {
  open_phic_item_category_form.value = true;
}

const closePhicItemCategoryForm = () => {
  open_phic_item_category_form.value = false;
}
