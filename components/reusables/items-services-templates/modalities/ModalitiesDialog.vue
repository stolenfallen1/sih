<template>
  ...
  <modalities-form
    :payload="payload"
    :open_modalities_form="open_modalities_form"
    @close-dialog="closeModalitiesForm"
...
const headers = [
  ...
  { title: "Modality", key: "modality", align: "start", width: "10%" },
  { title: "Description", key: "modilaty_description", align: "start", width: "40%" },
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "modalities?", "", params);
  if (response) {
...
const onSubmit = async (payload) => {
  ...
  if (payload.id) {
    response = await useMethod("put", "modalities", payload, "", payload.id);
  } else {
    response = await useMethod("post", "modalities", payload);
  }
  if (response) {
