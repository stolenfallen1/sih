<template>
      <v-dialog :model-value="show" rounded="lg" persistent scrollable max-width="750px">
        <v-card rounded="lg">
            <v-toolbar density="compact" color="#6984ff" hide-details>
                <v-toolbar-title>Medication Route</v-toolbar-title>
                <v-spacer></v-spacer>
    ...
    <medication-routes-form :payload="payload" :open_medication_routes_form="open_medication_routes_form" @close-dialog="closeMedicationRoutesForm" @handle-submit="onSubmit" />
...
const headers = [
    ...
    { title: 'Name', key: 'route_name', align: 'start',width:"20%" },
    { title: 'Description', key: 'route_description', align: 'start',width:"50%" },
...
const confirm = async () => {
    if(payload.value.id){
        let response = await useMethod("delete","delete-administration",payload.value,"",payload.value.id);
        if(response){
