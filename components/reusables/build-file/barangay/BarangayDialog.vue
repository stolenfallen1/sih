<template>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>New Barangay</v-toolbar-title>
          <v-spacer></v-spacer>
...
const headers = [
    ...
    { title: "Barangay", key: "barangay_name", width: "20%" },
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "get-barangay?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "update-barangay", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "create-barangay", payload.value);
  }
  if (response) {
