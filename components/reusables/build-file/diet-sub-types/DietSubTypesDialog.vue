  ...
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog"  scrollable max-width="750px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#107bac" hide-details>
        <v-toolbar-title>Diet Sub Types</v-toolbar-title>
        <v-spacer></v-spacer>
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "diet-sub-type?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "diet-sub-type", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "diet-sub-type", payload.value);
  }
  if (response) {
...
const confirm = async () => {
  if(payload.value.id){
      let response = await useMethod("delete","diet-sub-type", payload.value, "", payload.value.id);
      if(response) {
