<template>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Credit Card Details</v-toolbar-title>
          <v-spacer></v-spacer>
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "credit-cards?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "credit-cards", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "credit-cards", payload.value);
  }
  if (response) {
