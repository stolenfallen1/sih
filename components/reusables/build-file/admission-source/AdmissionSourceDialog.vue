<template>
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog" scrollable max-width="750px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Admission Source Names</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Admission Source Details</v-toolbar-title>
          <v-spacer></v-spacer>
        ...
        <v-divider></v-divider>
        <v-card-actions>
          <v-btn color="blue-darken-1 border border-info" @click="closeForm" v-on=""> Close </v-btn>
          <v-spacer></v-spacer>
...
const data = ref({
  title: "List of Admission Source",
  keyword: "",
  loading: false,
  filter: {},
  tab: 0,
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "admission-source?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "admission-source", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "admission-source", payload.value);
  }
  if (response) {
