<template>
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog" scrollable max-width="850px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Account Class</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Account Class Details</v-toolbar-title>
          <v-spacer></v-spacer>
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "account-classes", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "account-classes", payload.value);
  }
  if (response) {
