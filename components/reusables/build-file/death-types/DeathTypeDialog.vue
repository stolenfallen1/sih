<template>
  <v-dialog :model-value="show" rounded="lg" persistent scrollable max-width="850px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Death Type</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Death Type Details</v-toolbar-title>
          <v-spacer></v-spacer>
             ...
             <v-col cols="9">
              ...
              <v-col cols="12">
                <v-text-field
                  ...
                  density="compact"
                  required
                  v-model="payload.dohDeathTypes"
                  label="Enter dohDeathTypes"
                  clearable
                  hide-details
                ></v-text-field>
              </v-col>
...
const headers = [
  ...
  { title: "DOH Death Types", key: "dohDeathTypes", align: "start", width: "20%" },
  { title: "", key: "actions", align: "start", width: "20%" },
];
const data = ref({
  title: "List of Death Type",
  keyword: "",
  loading: false,
  filter: {},
  tab: 0,
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "death-type?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "death-type", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "death-type", payload.value);
  }
  if (response) {
