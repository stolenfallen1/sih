<template>
  <v-dialog :model-value="show" rounded="lg" persistent scrollable max-width="750px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Admission Type</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Admission Type Details</v-toolbar-title>
          <v-spacer></v-spacer>
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "admission-type?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "admission-type", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "admission-type", payload.value);
  }
  if (response) {
