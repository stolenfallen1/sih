<template>
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog"  scrollable max-width="750px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Doctor Category</v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-text-field
          label="Search by category_description"
          density="compact"
          variant="outlined"
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Doctor Category Details</v-toolbar-title>
          <v-spacer></v-spacer>
            ...
            <v-col cols="3">
             ...
             <v-col cols="9">
                <v-text-field
                  variant="outlined"
                  density="compact"
                  label="Enter category_description"
                  required
                  v-model="payload.category_description"
                  clearable
                  hide-details
                ></v-text-field>
              </v-col>
...
const headers = [
  ...
  { title: "category_description", key: "category_description", align: "start", width: "60%" },
  { title: "", key: "actions", align: "start", width: "20%" },
];

const data = ref({
  title: "List of doctor-categories",
  keyword: "",
  loading: false,
  filter: {},
  tab: 0,
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "doctor-categories?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "doctor-categories", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "doctor-categories", payload.value);
  }
  if (response) {
