<template>
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog"  scrollable max-width="700px">
      <v-card rounded="lg">
          <v-toolbar density="compact" color="#6984ff" hide-details>
              <v-toolbar-title>Other Hospital Services Templates</v-toolbar-title>
          ...
          <v-card-text>
              ...
              <v-data-table-server 
                  ...
                  <!-- <template v-slot:item.is_other_ancillary_procedure="{ item }">
                      {{ item.is_other_ancillary_procedure == 1 ? "Yes" : "No" }}
                  </template> -->
  ...
  <other-hospital-services-form :open_form_dialog="open_form_dialog" @close-dialog="closeFormDialog" @handle-submit="onSubmit" />
  <deleteConfirmation :show="confirmation" @confirm="confirm" @close="closeconfirmation" />
</template>

<script setup>
import OtherHospitalServicesForm from './sub-forms/OtherHospitalServicesForm.vue';
...
const headers = [
  ...
  { title: 'Grouping', key: 'grouping', align: 'start' },
  { title: 'Description', key: 'description', align: 'start', width:"60%" },
  // { title: '?', key: 'is_other_ancillary_procedure' , align: 'start' }, // Is Other Ancillary Procedure
