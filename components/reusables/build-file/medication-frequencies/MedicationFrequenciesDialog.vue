<template>
    <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog" scrollable max-width="750px">
        <v-card rounded="lg">
            ...
            <v-toolbar-title>Medication Frequencies Template</v-toolbar-title>
            <v-spacer></v-spacer>
    ...
    <v-dialog
        ...
        >
            <form @submit.prevent="onSubmit">
                <v-card rounded="lg">
                <v-toolbar density="compact" color="#6984ff" hide-details>
                    <v-toolbar-title>Medication Frequencies Details</v-toolbar-title>
                ...
                <v-card-text>
                    <v-row>
                        ...
                        <v-col cols="12">
                            <v-checkbox v-model="is_inactive" label="Inactive" hide-details density="compact"></v-checkbox>
                        </v-col>
                    </v-row>
                </v-card-text>
                <v-divider></v-divider>
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
    ...
    const response = await useMethod("get", "bed-status?", "", params);
    if (response) {
...
const onSubmit = async () => {
    ...
    // if (payload.value.id) {
    //     response = await useMethod("put", "bed-status", payload.value, "", payload.value.id);
    // } else {
    //     response = await useMethod("post", "bed-status", payload.value);
    // }
    // if (response) {
