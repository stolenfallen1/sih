<template>
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog"  scrollable max-width="750px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#107bac" hide-details>
        <v-toolbar-title>Therapeutic Class</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#107bac" hide-details>
          <v-toolbar-title>Therapeutic Class Details</v-toolbar-title>
          <v-spacer></v-spacer>
          ...
          <v-row>
            <v-col cols="12">
              <v-text-field
                ...
                placeholder="Enter Therapeutic name"
                hide-details
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "get-therapeutic-class?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "update-therapeutic-class", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "create-therapeutic-class", payload.value);
  }
  if (response) {
