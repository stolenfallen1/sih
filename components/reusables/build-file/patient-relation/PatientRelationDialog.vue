<template>
  <v-dialog :model-value="show" rounded="lg" persistent scrollable max-width="750px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Patient Relation</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Patient Relation Details</v-toolbar-title>
          <v-spacer></v-spacer>
...
const data = ref({
  title: "List of patient-relations",
  keyword: "",
  loading: false,
  filter: {},
  tab: 0,
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "patient-relations?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "patient-relations", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "patient-relations", payload.value);
  }
  if (response) {
