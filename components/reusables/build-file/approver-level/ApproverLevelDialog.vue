<template>
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog"scrollable max-width="750px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Approver Level</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Approver Level Details</v-toolbar-title>
          <v-spacer></v-spacer>
            ...
            <v-col cols="3">
             ...
             <v-col cols="9">
                <v-text-field
                  ...
                  required
                  v-model="payload.level_description"
                  clearable
                  hide-details
                ></v-text-field>
              </v-col>
...
const headers = [
  ...
  { title: "Description", key: "level_description", align: "start", width: "60%" },
  { title: "", key: "actions", align: "start", width: "20%" },
];

const data = ref({
  title: "List of refund-type",
  keyword: "",
  loading: false,
  filter: {},
  tab: 0,
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "get-approver-level?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "update-approver-level", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "create-approver-level", payload.value);
  }
  if (response) {
