<template>
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog"  scrollable max-width="750px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Suffix</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Suffix Details</v-toolbar-title>
          <v-spacer></v-spacer>
          ...
          <v-row>
            ...
            <v-col cols="6">
              <v-text-field
                variant="outlined"
                density="compact"
                label="Enter Abbreviations"
                required
                v-model="payload.abbreviations"
                clearable
                hide-details
              ></v-text-field>
            </v-col>
...
const headers = [
  ...
  { title: "Abbreviation", key: "abbreviations", align: "start", width: "20%" },
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "suffix?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "suffix", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "suffix", payload.value);
  }
  if (response) {
