<template>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#107bac" hide-details>
          <v-toolbar-title>Age Bracket Details</v-toolbar-title>
          <v-spacer></v-spacer>
...
const headers = [
  ...
  { title: "Min Age", key: "min_age", align: "start", width: "20%" },
  { title: "Max Age", key: "max_age", align: "start", width: "20%" },
  { title: "", key: "actions", align: "start" },
];
const data = ref({
  title: "List of Age Bracket",
  keyword: "",
  loading: false,
  filter: {},
  tab: 0,
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "age-bracket?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "age-bracket", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "age-bracket", payload.value);
  }
  if (response) {
