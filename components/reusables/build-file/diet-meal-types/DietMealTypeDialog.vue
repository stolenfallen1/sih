<template>
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog"  scrollable max-width="750px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Diet Meal Type</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Diet Meal Type Details</v-toolbar-title>
          ...
          <v-row>
              <v-col cols="12">
                <v-textarea
                  variant="outlined"
                  density="compact"
                  label="Diet Meal Description"
                  placeholder="Enter Diet Meal Description"
                  required
...
const headers = [
  ...
  { title: "Status", key: "isactive", align: "start", width: "20%"},
  { title: "", key: "actions", align: "start", width: "20%" },
];
const data = ref({
  title: "List of Diet Meal Types",
  keyword: "",
  loading: false,
  filter: {},
  tab: 0,
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "diet-meal-type?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "diet-meal-type", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "diet-meal-type", payload.value);
  }
  if (response) {
