    ...
    <v-card rounded="lg">
      ...
      <v-card-text>
        <v-row>
          <v-col cols="12">
            <v-text-field
              label="Search by Form Code / Name"
              density="compact"
...
const headers = [
  ...
  { title: "Form Name", key: "form_name", align: "start",sortable: false, width: "30%" },
  { title: "Form Code", key: "form_code", align: "start",sortable: false, width: "30%" },
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  let params =
    ...
    "&invgroup_id=" +
    inv_group;
  const response = await useMethod("get", "dosages?", "", params);
  if (response) {
...
const onSubmit = async (payload) => {
  ...
  if (payload.id) {
    response = await useMethod("put", "dosages", payload, "", payload.id);
  } else {
    response = await useMethod("post", "dosages", payload);
  }
  if (response) {
