<template>
  <v-dialog :model-value="show" rounded="lg" @update:model-value="closeDialog"  scrollable max-width="980px">
    <v-card rounded="lg">
      <v-toolbar density="compact" color="#6984ff" hide-details>
        <v-toolbar-title>Service Type</v-toolbar-title>
        <v-spacer></v-spacer>
  ...
  >
    <form @submit.prevent="onSubmit">
      <v-card rounded="lg">
        <v-toolbar density="compact" color="#6984ff" hide-details>
          <v-toolbar-title>Service Type Details</v-toolbar-title>
          <v-spacer></v-spacer>
             ...
             <v-col cols="9">
              ...
              <v-col cols="6">
                <v-text-field
                  variant="outlined"
                  density="compact"
                  label="Enter DOH Code"
                  required
                  v-model="payload.dohcode"
                  clearable
                  hide-details
                ></v-text-field>
              </v-col>
...
const headers = [
  ...
  { title: "Description", key: "description", align: "start", width: "20%" },
  { title: "Short Description", key: "short_description", align: "start", width: "20%" },
  { title: "DOH Code", key: "dohcode", align: "start", width: "20%" },
  { title: "Remarks", key: "remarks", align: "start", width: "10%" },
  { title: "", key: "actions", align: "start", width: "10%" },
];

const data = ref({
  title: "List of services-type",
  keyword: "",
  loading: false,
  filter: {},
  tab: 0,
...
const loadItems = async (page = null, itemsPerPage = null, sortBy = null) => {
  ...
  const response = await useMethod("get", "services-type?", "", params);
  if (response) {
...
const onSubmit = async () => {
  ...
  if (payload.value.id) {
    response = await useMethod("put", "services-type", payload.value, "", payload.value.id);
  } else {
    response = await useMethod("post", "services-type", payload.value);
  }
  if (response) {
