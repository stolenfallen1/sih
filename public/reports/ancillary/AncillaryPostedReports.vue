        ...
        <div style="margin-top: 8px;">
            <template v-if="groupedCharges">
                <template v-for="(charges, groupKey) in groupedCharges" :key="groupKey">
                    <p style="margin: 0; margin-top: 8px;">CENTRAL SUPPLY REFERENCE NO: 
                ...
                <div style="display: flex; justify-content: space-between; margin-top: 10px;">
                    <section style="display: flex; flex-direction: column;">
                        <p style="margin: 0; font-weight: bold;">Total Posted:
                            <span style="font-weight: 500;">{{ totalPosted }}</span>
                        </p>
                        <p style="margin: 0; font-weight: bold;">Total Returned:
                            <span style="font-weight: 500;">{{ totalReturned }}</span>
                        </p>
                        <!-- <p style="margin: 0; font-weight: bold;">Total Adjusted:
                            <span style="font-weight: 500;">{{ totalReturned }}</span>
                        </p> -->
                    </section>
                    <p style="font-weight: bolder;">
                        GRAND TOTAL:
                        <span style="border-bottom: 1px solid #000; font-weight: 500;">{{ usePeso(grandTotal) }}</span>
                    </p>
                </div>
...
const totalPosted = computed(() => {
    const totalPostedItems = props.charges.reduce((total, charge) => total + parseInt(charge.transaction_Qty), 0);
    return totalPostedItems;
})

const totalReturned = computed(() => {
    const totalReturnedItems = props.charges.reduce((total, charge) => total + parseFloat(charge.transaction_Qty < 0 ? charge.transaction_Qty : 0), 0);
    return Math.abs(totalReturnedItems);
})
