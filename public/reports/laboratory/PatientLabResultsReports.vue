        ...
        <div style="margin-top: 8px;">
            <template v-if="groupedCharges">
                <template v-for="(charges, groupKey) in groupedCharges" :key="groupKey">
                    <p style="margin: 0; margin-top: 8px;">LABORATORY REFERENCE NO: 
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin: 2px; text-align: left;">
                        <thead style="border-bottom: 1px solid #A9A9A9; border-top: 1px solid #A9A9A9;">
                            <tr>
                                ...
                                <th style="font-size: 12px; width: 10%;">EXAM ID</th>
                        ...
                        <tbody>
                            <tr v-for="(item, index) in charges" :key="index" style="border-bottom: 1px solid #A9A9A9;">
                                ...
                                <td style="font-size: 12px;">{{ 'LB' }}</td> <!-- Gi static kuni kay since lab reports raman sad ni, kay since sa view table gikan ang data's then walay revenueID didto. -->
                                <td style="font-size: 12px;">{{ item.itemid }}</td>
                                <td style="font-size: 12px;">
                                    {{ item.isrush == 'N' ? 'ROUTINE' : 'STAT' }}
                                </td>
                                <td style="font-size: 12px;">{{ item.exam }}</td>
                                <td style="font-size: 12px;">{{ parseInt(item.qty) }}</td>
                                <td style="font-size: 12px;">{{ usePeso(item.price) }}</td>
                            </tr>
                            <tr>
...
const processedBy = computed(() => {
    ...
    for (const [groupKey, charges] of Object.entries(groupedCharges.value)) {
        const firstProcessedBy = charges.find(charge => charge.renderby)?.renderby;
...
const groupSubTotal = (groupKey) => {
    ...
    return groupedCharges.value[groupKey].reduce((total, charge) => total + parseFloat(charge.price), 0);
};

const grandTotal = computed(() => {
    return Object.values(groupedCharges.value).reduce((total, charges) => {
        return total + charges.reduce((subTotal, charge) => subTotal + parseFloat(charge.price), 0);
    }, 0);
});
